...
def filter_parcels(img_indices, min_overlap):
	...
	with open('find_epicenter/252_epicenters/epicenter_parcelindices_dict_252_75hc.p', 'r') as f:
...
def create_epicenter_thr_seedmap_dict(candidates_list,
	...
	epicenter_thr_seedmap_dict : dict
		...
		connectivity map that are greater than or equal to the threshold.
	"""
	with open('find_epicenter/252_epicenters/epicenter_seedmap_dict_252_75hc.p', 'r') as f:
	...
	{i: percentile_threshold(filtered_epicenter_seedmap_dict[i],
	percentile_threshold_level) for i in filtered_epicenter_seedmap_dict.keys()}
...
def write_top_epicenters(img_path, img_indices, epicenter_thr_seedmap_dict,\
	...
	with open(outfile, 'a') as f:
		f.write('%s %s %s' % (img_path,
							  np.array(epicenters)[sorted_indices][::-1],
							  np.array(dice_coefs)[sorted_indices][::-1]))
		f.write('\n')
	lock.release()
